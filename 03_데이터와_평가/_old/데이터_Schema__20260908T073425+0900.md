...
| `track` | 문자열 | `controlled`(사람과 AI가 같은 brief를 받음) 또는 `parallel`(사람 문서에서 brief를 추출함). [병렬 말뭉치 계획](병렬_말뭉치_계획.md) 참고 |
| `brief_origin` | 문자열 | `authored`(Prompt Bank에서 작성) 또는 `derived-from-human`(사람 문서에서 추출) |
| `derived_from` | 문자열 또는 null | 병렬 트랙에서 brief를 추출한 사람 문서의 ID |
...
{"type": "wild-human", "origin_url": "https://...", "retrieved_at": "2026-09-01T10:00:00+09:00", "authorship_confidence": "medium", "authored_at_estimate": "2021년 게재일 기준"}
{"type": "hard-negative", "author_id": "auth-002", "fictional": true}
```

`type`은 `human-controlled`, `ai-controlled`, `wild-human`, `hard-negative` 네 가지다. Wild Human 문서에는 작성 주체의 불확실성을 `authorship_confidence`(`high`, `medium`, `low`)와 `authored_at_estimate`로 기록한다. 공개 웹 문서는 AI 보조로 쓰였을 수 있어 라벨이 틀릴 수 있기 때문이다. Hard Negative는 흔한 단서 표현을 정상적으로 쓰는 문서로 수집 시점에 분류한 것이며, 판정 결과가 정상이라는 보증이 아니다. 가상 예시와 수정한 예시는 `fictional: true`로 표시해 실제 원문과 구분한다.
...
| `track` | 문자열 | `controlled` 또는 `parallel`. 두 트랙의 통계는 분리해 보고한다. |
