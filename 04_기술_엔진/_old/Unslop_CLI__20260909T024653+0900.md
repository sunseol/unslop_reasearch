...
`unslop tui <file>`은 터미널 전체 화면에서 발견을 다룬다. 위쪽에 발견 목록(줄:열, severity, 규칙, 수정 가능 여부, 인용), 아래쪽에 선택한 발견의 규칙·메시지·수정 제안과 원문 문맥을 보여 준다. 발견 구간은 노란 배경으로, 재진술의 비교 구간은 파란 반전으로 강조한다.

| 키 | 동작 |
|---|---|
| ↑ ↓, j k, PgUp PgDn, Home End | 발견 이동 |
| a | 선택한 발견의 수정 적용 |
| A | 적용 가능한 수정 모두 적용 (최대 3회 반복) |
| u | 마지막 적용 되돌리기 |
| s | 파일에 저장 |
| w | warning만 보기 ↔ 전체 보기 |
| r | 다시 검사 |
| q | 종료. 저장하지 않은 수정이 있으면 한 번 더 눌러야 종료 |

수정을 적용하면 즉시 다시 검사해 좌표를 갱신한다. 저장은 s를 누를 때만 하며, 종료해도 파일은 바뀌지 않는다. 화면 그리기는 순수 함수로 분리되어 있어 테스트로 검증한다. Windows Terminal과 PowerShell 7에서 ANSI 색을 지원한다. 구형 cmd.exe에서는 색이 깨질 수 있다.
